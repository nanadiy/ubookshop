...
const Page: PageEl = (props, state:
{
  form:string,
  book:{
    title:string, author:string, country:string, imageLink:string, price:number, year:number, pages:number, language:string
  },
  cart:Array<string>
}
  ...
  return (
    <div style={{ direction: "rtl", minHeight: "11vh", }}>
      ...
      {state.form == "bookspecs" ? <WindowFloat
        ...
        state.cart.includes (state.book.title)?"#A34646":
        "#9ECA58"}}
         onClick={()=>{
      ...
      style={{ height:70, margin: 10, width: "calc(100% - 20px)" }}>

        <f-cse style={{width:"100%", height:45}}>

          <f-13>مجموع قابل پرداخت : {total_price.toLocaleString("fa-IR")} تومان</f-13>
          <f-13>تعداد کتاب ها : {state.cart.length.toLocaleString("fa-IR")} عدد </f-13>
